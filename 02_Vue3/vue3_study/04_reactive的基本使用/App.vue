...
export default defineComponent({
  ...
  setup() {
    ...
    // function updateUser
    const updateUser = () => {
      // obj.name = '哈哈哈'   不能直接用目标对象的方式更新目标对象中的成员值，数据不发生变化
      // user.name += '=='
      // user.age += 2
      // user.wife.name += '++'
      ...
      // obj.gender = '男' // 保存后不更新
      // user.gender = '女'   // 这种方式界面可以更新渲染

      // user对象或者obj对象中移除一个已经存在的属性，哪一种方式会影响界面的更新
      // delete obj.age // 界面更新

      // delete user.age
      // 通过当前的代理对象找到该对象中的某个属性，更改该属性中的某个数组中的数据
      // user.wife.cars[1] = '玛莎拉蒂'
      // 通过当前的代理对象，把目标对象中的某个数组属性添加一个新的属性
      user.wife.cars[3] = '奥拓'
    }
    return {
      user,
